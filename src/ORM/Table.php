...
class Table implements RepositoryInterface, EventListenerInterface, EventDispatcherInterface, ValidatorAwareInterface
{
     ...
     * ```
     *
     * @param string $type Named finder to invoke (e.g. `'all'`, `'list'`, `'published'`).
     *   Built-in finders: `'all'`, `'list'`, `'threaded'`. Custom finders are declared as
     *   `findXxx(SelectQuery $query, mixed ...$args): SelectQuery` on the table class.
     * @param mixed ...$args Named or positional arguments forwarded to the finder method.
     *   Named arguments map to finder parameter names; positional arguments map by order.
     * @return \Cake\ORM\Query\SelectQuery<\Cake\Datasource\EntityInterface|array> A lazy
     *   query builder. SQL is not executed until results are iterated or `all()` is called.
     * @complexity O(n) where n is the number of matching rows fetched from the database.
     * @see \Cake\ORM\Table::findAll() Default finder — returns all rows.
     * @see \Cake\ORM\Table::findList() Key-value list finder for select boxes.
     * @since 3.0.0
     */
    ...
    protected function _setFieldMatchers(array $options, array $keys): array
    {
        foreach ($keys as $field) {
            ...
            $options[$field] = function (array $row) use ($fields, $glue): string {
                $matches = [];
    ...
    /**
     * Fetch a single record by primary key, throwing if not found.
     *
     * Unlike `find()`, this method always returns an entity or throws —
     * it never returns null or false. Use it when a missing record is an
     * exceptional condition (e.g., a resource not found in a controller action).
     *
     * ### Usage
     *
     ...
     * ```
     *
     * Pass a custom finder and cache config:
     *
     * ```
     * $article = $articles->get(1, finder: 'published', cache: 'long');
     * ```
     *
     * @param mixed $primaryKey The primary key value (scalar or array for composite keys).
     *   For composite primary keys pass an array in the same column order as the schema.
     * @param array|string $finder The named finder to apply (e.g. `'published'`, `'all'`).
     *   Passing an options array is deprecated.
     * @param \Psr\SimpleCache\CacheInterface|string|null $cache PSR-16 cache instance or
     *   CakePHP cache config name. `null` disables result caching.
     * @param \Closure|string|null $cacheKey Explicit cache key. When `null` and `$cache`
     *   is set, a key is auto-generated from the table name and primary key value.
     * @param mixed ...$args Additional arguments forwarded to the named finder.
     * @return \Cake\Datasource\EntityInterface The matching entity, fully hydrated.
     * @throws \Cake\Datasource\Exception\RecordNotFoundException When no record matches
     *   the given primary key value.
     * @throws \Cake\Datasource\Exception\InvalidPrimaryKeyException When `$primaryKey` has
     *   a different number of elements than the table's defined primary key.
     * @see \Cake\Datasource\RepositoryInterface::find()
     * @complexity O(1) for simple primary-key lookups with a proper index; O(n) without.
     * @since 3.0.0
     */
    public function get(
        mixed $primaryKey,
     ...
     * @param TEntity $entity The entity to persist. Must implement EntityInterface.
     *   New entities (isNew() === true) generate an INSERT; existing ones generate an UPDATE
     *   only for dirty fields. If no fields are dirty the entity is returned unchanged.
     * @param array<string, mixed> $options Runtime save options:
     *   - `atomic` (bool, default true): wrap operation in a database transaction.
     *   - `associated` (bool|array, default true): save associated entities.
     *     Pass an array of association names to limit which associations are saved.
     *   - `checkRules` (bool, default true): run application-level rules before saving.
     *   - `checkExisting` (bool, default true): skip INSERT if record with same PK exists.
     * @return TEntity|false The saved entity on success (with auto-generated IDs populated).
     *   Returns `false` when validation errors are present, a rule check fails, a listener
     *   stops the `Model.beforeSave` event, or a database error occurs.
     * @throws \Cake\ORM\Exception\RolledbackTransactionException If an afterSave listener
     *   rolls back the wrapping transaction.
     * @see \Cake\ORM\Table::saveOrFail() Throws on failure instead of returning false.
     * @since 3.0.0
     */
