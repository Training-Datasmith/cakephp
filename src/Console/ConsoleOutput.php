...
class Console_Output
{
     ...
     */
    protected int $_output_as = self::COLOR;
    /**
     * text colors used in colored output.
     *
     * @var array<string, int>
     */
    protected static array $_foreground_colors = ['black' => 30, 'red' => 31, 'green' => 32, 'yellow' => 33, 'blue' => 34, 'magenta' => 35, 'cyan' => 36, 'white' => 37];
    /**
     * background colors used in colored output.
     *
     * @var array<string, int>
     */
    protected static array $_background_colors = ['black' => 40, 'red' => 41, 'green' => 42, 'yellow' => 43, 'blue' => 44, 'magenta' => 45, 'cyan' => 46, 'white' => 47];
    /**
    ...
    public function __construct($stream = 'php://stdout')
    {
        ...
        if (!is_resource($stream)) {
            throw new Console_Exception('Invalid stream in constructor. It is not a valid resource.');
        }
        $this->_output = $stream;
    ...
    public function write(array|string $message, int $newlines = 1): int
    {
        ...
        return $this->_write($this->style_text($message . str_repeat(static::LF, $newlines)));
    }
    /**
     ...
     */
    public function style_text(string $text): string
    {
        if ($this->_output_as === static::RAW) {
            return $text;
        }
        if ($this->_output_as !== static::PLAIN) {
            $replace_tags = $this->_replace_tags(...);
            $output = preg_replace_callback('/<(?P<tag>[a-z0-9-_.]+)>(?P<text>.*?)<\/(\1)>/ims', $replace_tags, $text);
            if ($output !== null) {
                return $output;
            }
        }
     ...
     */
    protected function _replace_tags(array $matches): string
    {
        $style = $this->get_style($matches['tag']);
        if (!$style) {
            return '<' . $matches['tag'] . '>' . $matches['text'] . '</' . $matches['tag'] . '>';
        }
        $style_info = [];
        if (!empty($style['text']) && isset(static::$_foreground_colors[$style['text']])) {
            $style_info[] = static::$_foreground_colors[$style['text']];
        }
        if (!empty($style['background']) && isset(static::$_background_colors[$style['background']])) {
            $style_info[] = static::$_background_colors[$style['background']];
        }
        unset($style['text'], $style['background']);
        foreach ($style as $option => $value) {
            if ($value) {
                $style_info[] = static::$_options[$option];
            }
        }
        return "\x1b[" . implode(';', $style_info) . 'm' . $matches['text'] . "\x1b[0m";
    }
    /**
     ...
     */
    public function get_output_as(): int
    {
        return $this->_output_as;
    }
    /**
     ...
     */
    public function set_output_as(int $type): void
    {
