...
use Cake\Database\Expression\Values_Expression;
...
class Insert_Query extends Query
{
    ...
    public function insert(array $columns, array $types = []): static
    {
        ...
        if (!$this->_parts['values']) {
            $this->_parts['values'] = new Values_Expression($columns, $this->get_type_map()->set_types($types));
        } else {
            /** @var \Cake\Database\Expression\ValuesExpression $valuesExpr */
            $values_expr = $this->_parts['values'];
            $values_expr->set_columns($columns);
        }
        return $this;
    }
    /**
     ...
     */
    public function values(Values_Expression|Query|array $data): static
    {
        if (empty($this->_parts['insert'])) {
            throw new Database_Exception('You cannot add values before defining columns to use.');
        }
        $this->_dirty();
        if ($data instanceof Values_Expression) {
            $this->_parts['values'] = $data;
            return $this;
        }
        /** @var \Cake\Database\Expression\ValuesExpression $valuesExpr */
        $values_expr = $this->_parts['values'];
        $values_expr->add($data);
        return $this;
    }
}