...
class Session
{
     ...
     */
    protected ?Session_Handler_Interface $_engine = null;
    /**
     ...
     */
    public static function create(array $session_config = []): static
    {
        if (isset($session_config['defaults'])) {
            $session_config = Hash::merge(static::_default_config($session_config['defaults']), $session_config);
        }
        if (!isset($session_config['ini']['session.cookie_secure']) && env('HTTPS') && ini_get('session.cookie_secure') != 1) {
            $session_config['ini']['session.cookie_secure'] = 1;
        }
        if (!isset($session_config['ini']['session.name']) && isset($session_config['cookie'])) {
            $session_config['ini']['session.name'] = $session_config['cookie'];
        }
        if (!isset($session_config['ini']['session.use_strict_mode']) && ini_get('session.use_strict_mode') != 1) {
            $session_config['ini']['session.use_strict_mode'] = 1;
        }
        if (!isset($session_config['ini']['session.cookie_httponly']) && ini_get('session.cookie_httponly') != 1) {
            $session_config['ini']['session.cookie_httponly'] = 1;
        }
        return new static($session_config);
    }
    /**
     ...
     */
    protected static function _default_config(string $name): array
    {
        ...
        if (!isset($defaults[$name])) {
            throw new Cake_Exception(sprintf('Invalid session defaults name `%s`. Valid values are: %s.', $name, implode(', ', array_keys($defaults))));
        }
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->configure_session_lifetime($lifetime);
        if ($config['cookie']) {
            $config['ini']['session.name'] = $config['cookie'];
        }
        if (!isset($config['ini']['session.cookie_path'])) {
            $cookie_path = empty($config['cookiePath']) ? '/' : $config['cookiePath'];
            $config['ini']['session.cookie_path'] = $cookie_path;
        }
        ...
        $this->_is_cli = PHP_SAPI === 'cli' || PHP_SAPI === 'phpdbg';
     ...
     */
    public function engine(Session_Handler_Interface|string|null $class = null, array $options = []): ?Session_Handler_Interface
    {
        if ($class === null) {
            return $this->_engine;
        }
        if ($class instanceof Session_Handler_Interface) {
            return $this->set_engine($class);
        }
        /** @var class-string<\SessionHandlerInterface>|null $className */
        $class_name = App::class_name($class, 'Http/Session');
        if ($class_name === null) {
            throw new InvalidArgumentException(sprintf('The class `%s` does not exist and cannot be used as a session engine', $class));
        }
        return $this->set_engine(new $class_name($options));
    }
    /**
     ...
     */
    protected function set_engine(Session_Handler_Interface $handler): Session_Handler_Interface
    {
    ...
    public function options(array $options): void
    {
        ...
        foreach ($options as $setting => $value) {
            if (ini_set($setting, (string) $value) === false) {
                throw new Cake_Exception(sprintf('Unable to configure the session, setting %s failed.', $setting));
            }
        }
    }
    /**
    ...
    public function start(): bool
    {
        ...
        if (session_status() === PHP_SESSION_ACTIVE) {
            throw new Cake_Exception('Session was already started');
        }
        $filename = null;
        $line = null;
        if (ini_get('session.use_cookies') && headers_sent($filename, $line)) {
            $this->header_sent_info = ['filename' => $filename, 'line' => $line];
            return false;
        }
        if (!session_start()) {
            throw new Cake_Exception('Could not start the session');
        }
        $this->_started = true;
        if ($this->_timed_out()) {
            $this->destroy();
    ...
    public function close(): bool
    {
        ...
        if (!session_write_close()) {
            throw new Cake_Exception('Could not close the session');
        }
        $this->_started = false;
        return true;
    }
    ...
    {
        if ($this->_has_session() && !$this->started()) {
            $this->start();
        }
        if (!isset($_SESSION)) {
            return false;
     ...
     * @return mixed|null
     */
    public function read_or_fail(string $name): mixed
    {
        if (!$this->check($name)) {
            throw new Cake_Exception(sprintf('Expected session key `%s` not found.', $name));
        }
    ...
    public function write(array|string $name, mixed $value = null): void
    {
        ...
        if (!$started) {
            ...
            if ($this->header_sent_info !== null) {
                $message .= sprintf(', headers already sent in file `%s` on line `%s`', Debugger::trim_path($this->header_sent_info['filename']), $this->header_sent_info['line']);
            }
            throw new Cake_Exception($message);
        }
        if (!is_array($name)) {
            $name = [$name => $value];
        }
    ...
    public function destroy(): void
    {
        ...
        if (!$this->_is_cli && session_status() === PHP_SESSION_ACTIVE) {
            session_destroy();
        }
        $_SESSION = [];
        $this->_started = false;
     ...
     */
    protected function _has_session(): bool
    {
        return !ini_get('session.use_cookies') || isset($_COOKIE[session_name()]) || $this->_is_cli || ini_get('session.use_trans_sid') && isset($_GET[session_name()]);
    }
    /**
    ...
    {
        if (!$this->_has_session() || $this->_is_cli) {
            return;
        }
        $this->start();
     ...
     */
    protected function _timed_out(): bool
    {
        $time = $this->read('Config.time');
        $result = false;
        $check_time = $time !== null && $this->_lifetime > 0;
        if ($check_time && time() - (int) $time > $this->_lifetime) {
            $result = true;
        }
     ...
     * @throws \Cake\Core\Exception\CakeException
     */
    public function set_session_lifetime(int $lifetime): void
    {
        if ($this->started()) {
            throw new Cake_Exception("Can't modify session lifetime after session has already been started.");
        }
        $this->configure_session_lifetime($lifetime);
    }
    /**
     ...
     */
    protected function configure_session_lifetime(int $lifetime): void
    {
        if ($lifetime !== 0) {
