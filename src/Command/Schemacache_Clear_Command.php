 ...
 */
class Schemacache_Clear_Command extends Command
{
    /**
    ...
    public function execute(Arguments $args, Console_Io $io): ?int
    {
        ...
        $tables = $cache->clear($args->get_argument('name'));
    ...
    {
        $parser->set_description([static::get_description(), 'If a table name is provided, only that table will be removed.'])->add_option('connection', ['help' => 'The connection to build/clear metadata cache data for.', 'short' => 'c', 'default' => 'default'])->add_argument('name', ['help' => 'A specific table you want to clear cached data for.', 'required' => false]);
        return $parser;
    }
}