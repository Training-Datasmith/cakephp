...
class Plural_Rules
{
    ...
    public static function calculate(string $locale, int $n): int
    {
        ...
        if (!isset(static::$_rules_map[$locale])) {
            $locale = explode('_', $locale)[0];
        }
        if (!isset(static::$_rules_map[$locale])) {
            return 0;
        }
        return match (static::$_rules_map[$locale]) {
            0 => 0,
            1 => $n === 1 ? 0 : 1,
            2 => $n > 1 ? 1 : 0,
            ...
            default => throw new Cake_Exception('Unable to find plural rule number.'),
        };
    }
}